...
class DSIAnalysisApp:
    ...
    def run(self, dsi_master_df, abstracts_file, op_api_key):
        ...
        if abstracts_file:
            try:
                ...
                if 'Abstract' not in df_abstracts.columns:
                    st.error("Input file must contain an 'Abstract' column")
                    st.write("Available columns:", list(df_abstracts.columns))
                    return
                
                st.write("Preview of abstracts to analyze:")
                st.dataframe(df_abstracts[['Abstract']].head())
                
                if st.button("Start Analysis"):
                    try:
                        # Process abstracts
                        
                        df_results = self.process_abstracts(df_abstracts)
