...
def setup(api_key):
    ...
    headers = ["Review source", "Rating", "Review Title", "Review Reaction", "Demographics", "Description", "Product Variant"]
...
def run(api_key, product_urls, output_folder_path, max_reviews=100):
    ...
    df = pd.DataFrame(columns=["Review source", "Rating", "Review Title", "Review Reaction", "Demographics", "Description", "Product Variant"])
    ...
    for product in products:
        ...
        for item in client.dataset(run["defaultDatasetId"]).iterate_items():
            ...
            ws.cell(row=row, column=5, value=str(demographics))
            ws.cell(row=row, column=6, value=str(desc))
            ws.cell(row=row, column=7, value=str(product_variant))
            
            row_data = ("Amazon", str(rating), str(review_title), str(review_reaction), str(demographics), str(desc), str(product_variant))
